...
const S=E.state,norm=v=>String(v||'').trim().toLocaleLowerCase(),openGroups=new Set(['__uncategorized__']);
...
function folderKey(category){return String(category||'').trim()||'__uncategorized__'}
function groupRows(container){
  ...
  const signature=rows.map(row=>{const p=(S.products||[]).find(x=>String(x?.code)===String(row.dataset.code));return row.dataset.code+':'+folderKey(p?.category)}).sort().join('|');
  ...
  rows.forEach(row=>{const p=(S.products||[]).find(x=>String(x?.code)===String(row.dataset.code)),key=folderKey(p?.category),label=key==='__uncategorized__'?'New / Uncategorized Products':String(p.category).trim();if(!groups.has(key))groups.set(key,{key,label,rows:[]});groups.get(key).rows.push({row,p})});
  const ordered=[...groups.values()].sort((a,b)=>a.key==='__uncategorized__'?-1:b.key==='__uncategorized__'?1:a.label.localeCompare(b.label,undefined,{sensitivity:'base',numeric:true}));
...
function setAllFolders(container,open){const body=container.querySelector('#p-body');if(!body)return;const keys=[...new Set((S.products||[]).map(p=>folderKey(p?.category)))];openGroups.clear();if(open)keys.forEach(key=>openGroups.add(key));else openGroups.add('__uncategorized__');delete body.dataset.folderSignature;groupRows(container)}
